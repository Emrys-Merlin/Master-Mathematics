...
\begin{thm}
  If \(X\) is a cube complex, then \(d\) is a metric.\todo{cite something}
\end{thm}
...
\begin{proof}
  ...
  For the second claim consider \(z \in X\) and \(0 < \epsilon < \epsilon(z)\) (as defined in\todo{cite BH}), such that \(f\) is a homeomorphism \(B \coloneqq B(z, \epsilon)\). Set \(\tilde B \coloneqq f(B)\) and let \(\tilde x, \tilde y \in \tilde B \cap \tilde C\), where \(\tilde C\) is some cube in \(Y\). Then we find unique preimages \(x\) and \(y\) in \(B\).
...
From now on we will also assume that all our CAT(0) cube complexes are locally countable.\todo{important perahps put at another location.}
