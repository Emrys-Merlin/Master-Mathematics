...
\begin{thm}[{\cite[I.7.10]{MR1744486}}]
  \label{thm:metric}
  Every cube complex \(X\) is a metric space, when equipped with metric \(d\) induced by the piecewise linear paths in \(X\).
\end{thm}

With the above definition we can define:

\begin{defin}[CAT(0) cube complex]
  A \emph{CAT(0) cube complex} is a cube complex \(X\) such that the pair \((X,d)\) is a CAT(0) space, where \(d\) is the metric induced by hte piecewise linear paths on \(X\).
\end{defin}
