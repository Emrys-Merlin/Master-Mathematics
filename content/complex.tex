...
\begin{rem}
  Altough the path metric \(d_p\) gives \(X\) its CAT(0) structure, this metric is often of no great importance in the theory of CAT(0) cube complexes. The reason for this is described in Remark~\ref{rem:geom-comb}.
\end{rem}
...
\end{bsp}
\begin{bsp}[Higher genus surfaces]
...
\begin{cor}
  \label{cor:non-inc}
  The restriction \(f|_C\colon C \to Y\) is distance non-increasing for each cube \(C \subset X\), i.\,e.\ \(d_Y(f(x), f(y)) \leq d_X(x,y)\) for all \(x,y \in C\).
\end{cor}
