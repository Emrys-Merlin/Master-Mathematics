...
\begin{prop}[{\cite[Proposition~4.17]{MR3509968}}]
  \label{prop:4.17}
  Let \(X\) be a CAT(0) cube complex and \(\Gamma \to \Aut(X)\) an essential action. Let \(\mathcal{H}' \subset \mathcal{H}(X)\) be a \(\Gamma\)-invariant subset of halfspaces and \(X_\alpha \subset X\) a \(\Gamma\)-invariant family (with more than one member) of subcomplexes such that \(\mathcal{\hat H}(X_\alpha) = \mathcal{\hat H}'\). Let \(Y\) be the smallest strongly convex subcomplex containing \(A \coloneqq \cup_\alpha X_\alpha\). Then \(Y = X\) and \(X = \overline{X(\mathcal{\hat H}')} \times Z\), where \(Z\) is some other CAT(0) cube complex.
\end{prop}
...
\begin{thm}[{\cite[Theorem~4.1]{MR3509968}}]
  \label{thm:4.1}
  Let \(\Gamma\) be a discrete, countable group acting on an finite dimensional CAT(0) cube complex \(X\) via automorphisms. Assume the action is essential and non-elementary. If \(B\) is a strong \(\Gamma\)-boundary, there exists a measurable map
  \[
...
\begin{proof}
  ...
  \item[\(\vartheta(\mathcal{E})=1\):] Since \(H_\mu = H_\nu\) for almost all \(\mu\) and \(\nu\) and by the definition of the product measure we can find a \(\mu_0 \in \mathcal{P}(\bar X)\) and a full measure subset \(M \subset \mathcal{P}(X)\), such that \(H_\nu = H_{\mu_0}\) for all \(\nu \in M\). By Lemma~\ref{lem:countable-orbit} we find a \(\nu \in M\) such that
    ...
    \[
      X = X(H_\mu) \times Z.
    \]
    However, \(X(H_\mu)\) is an interval. Clearly, every of the irreducible factors in \(X(H_\mu)\) are also intervals (just project the ultrafilters to the factors). However, this is a contradiction to the non-elementarity of \(\Gamma\)-action by Corollary~\ref{cor:f-3.21}.
  \item[\(\vartheta(\mathcal{E})=0\):] In this case we have \(H_\mu \neq H_\nu\) for almost all \(\mu\) and \(\nu\). We define the set
    ...
    \item If the essential value is 0, i.\,e.\ \(H_\mu\) does not contain strongly separated halfspaces for almost all \(\mu\). Let
      \[
...
Before finishing this treatise, we would like to generalize our main theorem. We would both like to lessen the restrictions on \(\Gamma\) by only assuming no finite orbits in the visual boundary and on \(X\) by allowing reducibility. It turns out that this is possible. However, we will have to allow for two drawbacks. First, \(\phi\) will only be equivariant with regard to a finit index subgroup of \(\Gamma\). Secondly, the image of \(\phi\) might not lie in the non-terminating ultrafilters, but it will still lie in \(\partial X\). The precise statement is as follows:
