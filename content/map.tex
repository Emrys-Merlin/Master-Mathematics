...
\begin{proof}
  ...
  \begin{description}
  \item[\(D = 1\):] By Lemma~\ref{lem:A.6} we can find a \(v_1 \in V\) such that
    \[
...
\begin{proof}
  ...
  \]
  and since \(\mathfrak{h}_2\) and \(\mathfrak{h}_3\) are strongly separated, by the Lemma~\ref{lem:strongly-sep} we have that the above halfspaces are pairwise strongly separated.
...
\begin{thm}[{\cite[Theorem~4.1]{MR3509968}}]
  \label{thm:4.1}
  Let \(\Gamma\) be a discrete, countable group acting on a connectee, locally countable, finite-dimensional CAT(0) cube complex \(X\) via automorphisms. Assume the action is essential and non-elementary. If \(B\) is a strong \(\Gamma\)-boundary there exists a measurable map
  \[
