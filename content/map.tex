...
\begin{proof}
  ...
  \item[\(\N_C = N_1 \sqcup N_3\):] If \(N_3 \neq \varnothing\), we take \((n_0, m_0) \in N_3\) and define
    ...
    \end{align*}
    \(M\) is well-defined since the maximum is taken over a non-empty set (by choice of \((n_0, m_0)\)) and the set is finite since two nested halfspaces contain only finitely many halfspaces in between.\todo{perhaps cite this result}
...
\begin{defin}
  A \emph{directed graph \(\mathcal{G}(V,E)\)} consists of two sets \(V\) and \(E\) which are denoted its \emph{vertex set} and \emph{edge set} respectively and of two maps \(s, t \colon E \to V\) associating to each edge its source and target respectively. In our case there are no multiple edges possible and we will not allow loops. We will call \(\mathcal{G}\) \emph{complete}, if it is complete as an undirected graph, i.\,e.\ each vertex is connected via an edge to each other vertex. For each \(v \in V\) we will by \(o(v) \subset E\) denote the set of \emph{outgoing edges} and by \(i(v) \subset E\) the set of \emph{incoming edges}. We will call a complete directed finite Graph \emph{upper triangular}, if there exist a numbering \(V = \{v_1, \dots, v_D\}\), such that for all \(j = 1, \dots, D\) we have
  \begin{align*}
...
\begin{proof}
  ...
  \end{align*}
  Fixing \(n = C\) and for each \(m \geq M\) we have that \(\mathfrak{\hat h}_n^i \pitchfork \mathfrak{\hat h}_m^j\) for each \(1 \leq i < j \leq D\). Fixing \(m\) this leads to a set of \(D\) transverse halfspaces. Since these intersect pairwise, we know\todo{cite} that the commen intersection is not empty and an element in this intersection is in a cube, which has all these hyperplanes as midcubes. Hence this cube has at least dimension \(D\). Since \(D\) was arbitrary this shows, that \(X\) contains cubes of arbitrary dimension.
\end{proof}
...
\begin{proof}
  ...
  \]
  and since \(\mathfrak{h}_2\) and \(\mathfrak{h}_3\) are strongly separated, we can show that all the above halfspaces are pairwise strongly separated.\todo{add a short lemma for this}
...
\begin{thm}[{\cite[Theorem~4.1]{MR3509968}}]\todo{irreducible}\todo{essential action}\todo{non-elementary action}\todo{strong \(\Gamma\)-boundary}\todo{non-terminating ultrafilters}
  \label{thm:4.1}
